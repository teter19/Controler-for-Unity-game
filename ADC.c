...
uint8_t ADC_Init(void)
{
	...
	SIM->SCGC5 |= SIM_SCGC5_PORTA_MASK;					// Dołączenie sygnału zegara do portu B
	PORTA->PCR[2] &= ~(1<<2);										//PTB2 - wejście analogowe, kanał 4
	PORTA->PCR[1] &= ~(1<<2);										//PTB2 - wejście analogowe, kanał 5, chyba raczej PTB1
